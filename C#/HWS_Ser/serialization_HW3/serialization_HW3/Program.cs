...
namespace Journal
{
    ...
    class Magazines_list
    {
        Magazines_list(List<Magazine> magazines)
        {
            this.Magazines = magazines;
        }

        public Magazines_list CreateMagazinesList()
        {
            List<Magazine> list = new List<Magazine>();

            for (int i = 0; i < list.Count; i++)
            {
                list[i] = list[i].CreateMagazine();
            }

            Magazines_list tmpList = new Magazines_list(list);
            return tmpList;
        }

        public void printAll()
        {
            for (int i = 0; i < this.Magazines.Count; i++)
            {
                this.Magazines[i].printAll();
            }
        }
